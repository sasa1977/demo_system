defmodule ExampleSystem.Workers do
  ...
  def start_worker(id), do:
    Supervisor.start_child(supervisor_name(rem(id, @shards) + 1), [id])

  defp number_producer_supervisors(), do:
    Enum.map(1..@shards, &number_producer_sup/1)
  ...
  defp supervisor_name(id), do:
    Module.concat(__MODULE__.Supervisor, :"#{id}")
